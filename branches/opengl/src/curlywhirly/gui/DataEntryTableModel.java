...
public class DataEntryTableModel extends AbstractTableModel
{
	...
	DataEntryTableModel(DataSet dataSet, ClassificationScheme currentScheme)
	{
		this.dataSet = dataSet;

		dataEntries = new ArrayList<DataEntry>();
		int categorySchemeIndex =  dataSet.classificationSchemes.indexOf(currentScheme);
		for (DataEntry dataEntry : dataSet.dataEntries)
		{
			Category category = dataEntry.categories.get(categorySchemeIndex);

			if (category != null && category.highlight)
